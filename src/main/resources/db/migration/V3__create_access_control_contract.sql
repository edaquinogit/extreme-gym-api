...
create table auditoria_acesso (
    ...
    constraint fk_auditoria_acesso_usuarios foreign key (usuario_id) references usuarios (id),
    constraint fk_auditoria_acesso_dispositivos foreign key (dispositivo_id) references dispositivos_acesso (id)
);
...
create index idx_eventos_acesso_data_hora on eventos_acesso (data_hora_evento);
create index idx_eventos_acesso_resultado on eventos_acesso (resultado);
...
create index idx_credenciais_acesso_tipo_identificador on credenciais_acesso (tipo, identificador_externo);
